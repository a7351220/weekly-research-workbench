...
function isStrongNewsCandidate(candidate: StoryCandidate): boolean {
  ...
  if (/\b(pre-market earnings report|after-hours earnings report|earnings report for may)\b/i.test(candidate.title)) return false;
  if (/\b(parloa|gardening tips|future vision film competition)\b/i.test(text)) return false;
